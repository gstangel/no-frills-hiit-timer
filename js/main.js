// disclaimer: this is one of my first independant coding projects.  This
// code is shotty to say the least
...
roundsSlider.oninput = function () {
  ...
  //for audio playback when round starts or ends

}

//loading audio snippits into memory
...
function startTimer() {
  ...
  let roundInput = parseInt(document.getElementById("rounds-slider").value);
  var lowIntensityVar = lowIntensitySec;
  var highIntensityVar = highIntensitySec;
  //bool to determine which cycle is active
  var isLow = true;
  ...
  function updateTimer() {
    document.getElementById("rounds-left").innerHTML = "Rounds Remaining: " + (roundInput - roundsCounter);
    //logic to determine if rounds have been completed
    if (roundsCounter < roundInput) {
      if (isLow == true) {
        if (soundPlayed == false) {
          soundPlayed = true;
          lowIntensity.play();
        }
        //update styling to reflect active intensity
        ...
        //when there is still time left in the current low intensity round, reflect that to the user
        ...
        //play high intensity sound when round intensity changes
        if (soundPlayed == false) {
          soundPlayed = true;
          highIntensity.play();
        }
  ...
  //call the function every 10ms to go through logic flow for the timer
  function startTimer() {
